...
class CustomerRequests extends \yii\db\ActiveRecord
{
    ...
    public static function findRequestsInfo($orderBy = null)
    {
        if ($orderBy  && is_array($orderBy) )
        {
           ...
           return static::find()
               ->innerJoin("customer", "customer.`id` = ". static::tableName() . ".customer_id")
               ->innerJoin("person", "person.`id` = customer.person_id")
               ->leftJoin("address", "address.`person_id` =  person.`id`")
               ->leftJoin("company", "company.`id` = customer.company_id")
               ->where(['customer.deleted' => GLOBALS::UNDELETED])
               ->orderBy($orderBy);
